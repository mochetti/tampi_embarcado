...
void starWars() {
  Serial.println("MUSICA: star wars");
  ...
  int notes = sizeof(starWarsNotas) / sizeof(starWarsNotas[0]) / 2;
  // iterate over the notes of the melody.
  // Remember, the array is twice the number of notes (notes + durations)
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {

    // calculates the duration of each note
    divider = starWarsNotas[thisNote + 1];
    if (divider > 0) {
    ...
    tone(buzzerPin, starWarsNotas[thisNote], noteDuration * 0.9);
...
void asaBranca() {
  Serial.println("MUSICA: asa branca");
  // change this to make the song slower or faster
  int tempo = 108;
  ...
  // sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
  // there are two values per note (pitch and duration), so for each note there are four bytes
  int notes = sizeof(asaBrancaNotas) / sizeof(asaBrancaNotas[0]) / 2;
  // iterate over the notes of the melody.
  // Remember, the array is twice the number of notes (notes + durations)
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {

    // calculates the duration of each note
    divider = asaBrancaNotas[thisNote + 1];
    if (divider > 0) {
      // regular note, just proceed
      noteDuration = (wholenote) / divider;
    } else if (divider < 0) {
    ...
    // we only play the note for 90% of the duration, leaving 10% as a pause
    tone(buzzerPin, asaBrancaNotas[thisNote], noteDuration * 0.9);
...
void pantera() {
  ...
  // sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
  // there are two values per note (pitch and duration), so for each note there are four bytes
  int notes = sizeof(panteraNotas) / sizeof(panteraNotas[0]) / 2;
  // iterate over the notes of the melody.
  // Remember, the array is twice the number of notes (notes + durations)
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {

    // calculates the duration of each note
    divider = panteraNotas[thisNote + 1];
    if (divider > 0) {
      // regular note, just proceed
      noteDuration = (wholenote) / divider;
    } else if (divider < 0) {
    ...
    // we only play the note for 90% of the duration, leaving 10% as a pause
    tone(buzzerPin, panteraNotas[thisNote], noteDuration * 0.9);
